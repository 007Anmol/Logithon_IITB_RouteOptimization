...
import { 
  ...
  FileSpreadsheet, Download, Package, Loader2, X,
  ArrowRight, Info, FileCog, Database, FileCheck
} from "lucide-react"
...
const initialProcessSteps: ProcessStep[] = [
  { id: "upload", name: "Upload CSV", description: "Upload your shipment data file", status: "pending", percentage: 0 },
  { id: "validate", name: "Validate Data", description: "Check data format and constraints", status: "pending", percentage: 0 },
  { id: "optimize", name: "Optimize Routes", description: "Calculate optimal shipping routes", status: "pending", percentage: 0 },
  { id: "complete", name: "Generate Results", description: "Prepare visualization and reports", status: "pending", percentage: 0 }
];
...
export const CsvUploadForm: FC = () => {
  ...
  const [parsedData, setParsedData] = useState<ShipmentUploadResult | null>(null)
              ...
              try {
                ...
                if (!row.origin || !row.destination || !row.weight) {
                  errors.push({ 
                    ...
                    message: "Missing required field (origin, destination, or weight)"
                  })
                  return
                }
                ...
                const shipment: Shipment = {
                  ...
                  hazardous: row.hazardous === "true" || row.hazardous === true || false,
                  temperature: row.temperature 
                    ? { 
                        min: typeof row.temperature.min === 'number' ? row.temperature.min : 0, 
                        max: typeof row.temperature.max === 'number' ? row.temperature.max : 30 
                      }
                    : undefined
                }
    ...
    setTimeout(() => {
      const validationProgress = setInterval(() => {
        setProcessSteps(prev => {
          ...
          const validateStep = updatedSteps.find(step => step.id === "validate")
  ...
  const downloadSampleCsv = (): void => {
    const sampleData = [
      ["origin", "destination", "weight", "dimensions", "description", "priority", "deadline", "fragile", "hazardous"],
      ["Los Angeles, USA", "New York, USA", "450", "50x40x30", "Electronics", "high", "2023-12-25", "true", "false"],
      ["New York, USA", "London, UK", "1200", "120x80x75", "Machinery parts", "medium", "2023-12-30", "false", "false"],
      ["Los Angeles, USA", "Tokyo, Japan", "850", "90x60x40", "Medical supplies", "high", "2023-12-20", "true", "false"],
      ["New York, USA", "Mumbai, India", "1500", "100x80x60", "Industrial equipment", "medium", "2024-01-15", "false", "false"],
      ["London, UK", "Singapore", "720", "85x55x45", "Fashion merchandise", "low", "2024-01-10", "false", "false"],
      ["Tokyo, Japan", "Sydney, Australia", "950", "110x70x50", "Automotive parts", "medium", "2024-01-05", "false", "true"],
      ["Shanghai, China", "Frankfurt, Germany", "1100", "130x90x70", "Consumer electronics", "high", "2023-12-28", "true", "false"],
      ["Dubai, UAE", "São Paulo, Brazil", "1300", "140x95x80", "Oil equipment", "medium", "2024-01-20", "false", "true"],
      ["Mexico City, Mexico", "Madrid, Spain", "680", "75x50x40", "Textiles", "low", "2024-01-25", "false", "false"],
      ["Hong Kong", "Paris, France", "790", "85x60x50", "Luxury goods", "high", "2023-12-22", "true", "false"],
      ["Toronto, Canada", "Delhi, India", "980", "100x70x60", "Agricultural machinery", "medium", "2024-01-18", "false", "false"],
      ["Seoul, South Korea", "Moscow, Russia", "1050", "120x80x70", "Electronics components", "medium", "2024-01-12", "true", "false"],
      ["Bangkok, Thailand", "Cairo, Egypt", "870", "95x65x55", "Food products", "high", "2023-12-18", "false", "false"],
      ["Jakarta, Indonesia", "Amsterdam, Netherlands", "1150", "130x85x75", "Furniture", "low", "2024-01-28", "false", "false"]
    ]
  ...
  const currentStep = processSteps.find(step => step.status === "processing")?.id || 
      ...
      >
        ...
        {optimizationStatus !== "idle" && (
          <div className="mb-6">
            <div className="flex mb-2 justify-between items-center">
              <h3 className="text-sm font-medium text-white">Optimization Progress</h3>
              <span className="text-xs text-blue-200/70">
                {optimizationStatus === "complete" ? "Completed" : "Processing..."}
              </span>
            </div>
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-blue-200/50">{step.description}</p>
                      </div>
                    </div>
                    <span className="text-xs text-blue-200/70">{step.percentage}%</span>
                  </div>
                  <Progress 
        ...
        {parsedData && (
          <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-200/70">
                    ...
                    {parsedData.invalidCount > 0 && ` (${parsedData.invalidCount} errors)`}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
            ...
            {showDataPreview && (
              <div className="mb-4 p-4 rounded-lg border border-white/10 bg-white/5 max-h-60 overflow-auto">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-white/20">
                        <th className="px-2 py-1 text-left font-medium text-blue-200/80">ID</th>
                        <th className="px-2 py-1 text-left font-medium text-blue-200/80">Origin</th>
                        <th className="px-2 py-1 text-left font-medium text-blue-200/80">Destination</th>
                        <th className="px-2 py-1 text-left font-medium text-blue-200/80">Weight</th>
                        <th className="px-2 py-1 text-left font-medium text-blue-200/80">Dimensions</th>
                        <th className="px-2 py-1 text-left font-medium text-blue-200/80">Priority</th>
                      </tr>
                    </thead>
                    <tbody>
                      {parsedData.shipments.slice(0, 5).map((shipment) => (
                        <tr key={shipment.id} className="border-b border-white/10">
                          <td className="px-2 py-1 text-blue-200/60">{shipment.id.slice(0, 10)}</td>
                          <td className="px-2 py-1 text-blue-200/60">{shipment.origin}</td>
                          <td className="px-2 py-1 text-blue-200/60">{shipment.destination}</td>
                          <td className="px-2 py-1 text-blue-200/60">{shipment.weight} kg</td>
                          <td className="px-2 py-1 text-blue-200/60">{shipment.dimensions}</td>
                ...
                {parsedData.errors.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-xs font-medium text-yellow-400 mb-2">Errors & Warnings</h4>
            ...
            <Alert variant="default" className="mb-4 bg-blue-900/20 border-blue-500/30">
              <Database className="h-4 w-4 text-blue-400" />
              <AlertTitle className="text-sm font-medium text-white">Ready to optimize</AlertTitle>
              <AlertDescription className="text-xs text-blue-200/70">
                {parsedData.validCount} shipments will be processed for route optimization. This will identify cost-saving 
                opportunities through clustering, multi-modal transport, and backhaul optimization.
              </AlertDescription>
            </Alert>
          </div>
        )}
        ...
        >
          ...
          <Button 
            type="submit" 
            disabled={!parsedData || isUploading || optimizationStatus !== "idle"} 
        ...
        >
          ...
          <ul className="list-disc pl-5 space-y-1 text-xs">
            ...
            <li>Optional columns: dimensions, description, priority, deadline, fragile, hazardous</li>
          ...
          <Tabs defaultValue="preview" className="w-full">
            ...
            <TabsContent value="preview" className="mt-4">
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      <tr className="border-b border-white/10">
                        <td className="px-2 py-1 text-blue-200/60">Los Angeles, USA</td>
                        <td className="px-2 py-1 text-blue-200/60">New York, USA</td>
                      ...
                      <tr className="border-b border-white/10">
                        <td className="px-2 py-1 text-blue-200/60">New York, USA</td>
                        <td className="px-2 py-1 text-blue-200/60">London, UK</td>
                        <td className="px-2 py-1 text-blue-200/60">1200</td>
                        <td className="px-2 py-1 text-blue-200/60">120x80x75</td>
                        <td className="px-2 py-1 text-blue-200/60">medium</td>
                      </tr>
                      <tr className="border-b border-white/10">
                        <td className="px-2 py-1 text-blue-200/60">Los Angeles, USA</td>
                        <td className="px-2 py-1 text-blue-200/60">Tokyo, Japan</td>
              ...
              <p className="mt-4 text-xs text-blue-200/70">
                This is a preview of how your CSV data should be formatted. The headers are required, 
                and each row represents a single shipment to be optimized.
              </p>
            </TabsContent>
              ...
              <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="p-2 rounded bg-white/10 text-xs">
                    ...
                    <p className="text-[10px] text-blue-200/60 mt-1">Pickup location</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
                    <span className="font-medium">destination</span>
                    <p className="text-[10px] text-blue-200/60 mt-1">Delivery location</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
                    <span className="font-medium">weight</span>
                    <p className="text-[10px] text-blue-200/60 mt-1">Weight in kg</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 rounded bg-white/10 text-xs">
                    ...
                    <p className="text-[10px] text-blue-200/60 mt-1">Format: LxWxH in cm</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
                    <span className="font-medium">description</span>
                    <p className="text-[10px] text-blue-200/60 mt-1">Item description</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
                    <span className="font-medium">priority</span>
                    <p className="text-[10px] text-blue-200/60 mt-1">low, medium, high</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
                    <span className="font-medium">deadline</span>
                    <p className="text-[10px] text-blue-200/60 mt-1">Format: YYYY-MM-DD</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
                    <span className="font-medium">fragile</span>
                    <p className="text-[10px] text-blue-200/60 mt-1">true/false</p>
                  </div>
                  <div className="p-2 rounded bg-white/10 text-xs">
